...
const ArticleCard = ({ article }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-6 transition-shadow hover:shadow-lg">
      ...
      <p className="text-gray-600 leading-relaxed mb-5 line-clamp-3">{article.summary}</p>
      <a 
      ...
      >
        Read Full Article
        <span className="transition-transform group-hover:translate-x-1"><ExternalLinkIcon /></span>
      </a>
    </div>
  );
};
    ...
    <div className="min-h-screen bg-slate-100 font-sans text-slate-800">
      <div className="container mx-auto p-4 md:p-8">
        <header className="text-left mb-10 pb-4 border-b border-slate-300">
          ...
          <p className="text-md text-slate-500 mt-1">Your Personal AI News Curator</p>
        </header>
          ...
          <aside className="lg:col-span-1">
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 sticky top-8">
              ...
              <ul className="space-y-2">
                <li>
                  <button onClick={() => handleCategorySelect('general')} className={`w-full text-left px-3 py-2 rounded-md text-sm font-medium transition-colors ${ selectedCategory === 'general' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-slate-100'}`}>
                    All News
                  </button>
                </li>
                ...
                {categories.map((cat) => (
                  <li key={cat.id} className="flex items-center justify-between group">
                    <button onClick={() => handleCategorySelect(cat.category_name)} className={`flex-grow text-left px-3 py-2 rounded-md text-sm font-medium transition-colors ${ selectedCategory === cat.category_name ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-slate-100'}`}>
                      {cat.category_name.charAt(0).toUpperCase() + cat.category_name.slice(1)}
                    </button>
                    <button onClick={() => handleRefresh(cat.category_name)} className="ml-2 p-1.5 rounded-md hover:bg-slate-200 text-gray-500 hover:text-blue-600 transition-colors" title={`Refresh ${cat.category_name}`}>
              ...
              <div className="mt-6 pt-6 border-t border-gray-200">
                  <form onSubmit={handleAddCategory}>
                    ...
                    >
                      {loading.refresh && loading.refresh === newCategory.trim().toLowerCase() ? <Spinner size="h-4 w-4" /> : 'Add Category'}
                    </button>
                  </form>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {loading.articles ? (
                ...
                <div className="text-center bg-white p-12 rounded-lg shadow-sm border">
                  ...
                  <p className="text-gray-500 mt-2">There are no articles available for "{selectedCategory}".</p>
                </div>
              )}
